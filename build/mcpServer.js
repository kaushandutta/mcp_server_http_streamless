...
export class MCPServer {
    ...
    static setUpTools(server) {
        // Register the tool - the SDK will automatically handle tools/list
        server.tool("get-weather-data", "Get the weather data for a given city", {
    ...
    static async cityWeather(city) {
        try {
            const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=b5e1905f70ef4bf47b3235d50242e204&units=metric`);
